...
def get_argument_elements(source_data_folder, processed_data_folder): 
    ...
    def process_json_file(json_file_path):
        ...
        try:
            ...
            relation_nodes = [n for n in data['nodes'] if n.get('type') in ['CA', 'RA']]

            # Extract I nodes
            info_nodes = [n for n in data['nodes'] if n.get('type') == 'I']
    ...
    for root, dirs, files in os.walk(source_data_folder):
        for filename in files:
            if filename.endswith('.json'):
                ...
                result_df = process_json_file(json_file_path)
